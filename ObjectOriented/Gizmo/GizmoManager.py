...
from ObjectOriented.Gizmo.RotationGizmo import RotationGizmo
    ...
    def AddGizmo(self, Entity, GizmoType: str, OnGizmoAdded=None):
        ...
        match GizmoType:
            case "PositionGizmo":
                ...
                self.CurrentGizmo = PositionGizmo(Entity, Snapping=self.PositionSnapping, OnDrag=self.OnDrag,
                                                  parent=self)
                self.CurrentGizmo.SetUp()

            case "RotationGizmo":
                self.RemoveGizmo()
                self.CurrentGizmo = RotationGizmo(Entity, Snapping=self.RotationSnapping, OnDrag=self.OnDrag,
                                                  parent=self)
                self.CurrentGizmo.SetUp()
