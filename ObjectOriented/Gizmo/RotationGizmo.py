...
from ursina import *
from ObjectOriented.Gizmo.GizmoUpdater import NewDraggable
import site
...
class RotationGizmo(Entity):
    def __init__(self, Entity, OnDrag, Snapping=0, **kwargs):
        ...
        self.GizmoX = NewDraggable(name="rotation_x", Num=0, parent=self, RetVal=self.GetVal, Entity=Entity,
                                   lock=(0, 1, 1), scale=(1, 1, 1), rotation=(90, 90, 0), color=color.red,
                                   model=f"../../Assets/Models/ScaleGizmo.obj", collider="mesh", double_sided=True,
        ...
        self.GizmoY = NewDraggable(name="rotation_y", Num=1, parent=self, RetVal=self.GetVal, Entity=Entity,
                                   lock=(1, 0, 1), scale=(1, 1, 1), rotation=(0, 90, 0), color=color.blue,
                                   model=f"../../Assets/Models/ScaleGizmo.obj", collider="mesh", double_sided=True,
                                   step=self._Snapping, position=Entity.position)
        self.GizmoZ = NewDraggable(name="rotation_z", Num=2, parent=self, RetVal=self.GetVal, Entity=Entity,
                                   lock=(1, 0, 0), plane_direction=(0, 1, 0), rotation=(90, 90, 90), scale=(1, 1, 1),
                                   color=color.green, model=f"../../Assets/Models/ScaleGizmo.obj", collider="mesh",
